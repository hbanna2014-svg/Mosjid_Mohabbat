...
export default function App() {
  ...
  const playNotification = (message: string, isAlarm: boolean = false) => {
    ...
    if (Notification.permission === 'granted') {
      new Notification('মসজিদ মহব্বত (Masjid Mohabbat)', {
        body: message,
        icon: '/favicon.ico'
      });
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(mosquesRef, (snapshot) => {
      ...
      // Update persistent cache for offline mode
      localStorage.setItem('mosques_cache', JSON.stringify(mosqueList));
          ...
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full border-2 border-white/20 overflow-hidden bg-white/10 flex items-center justify-center">
              <img 
                ...
                onError={(e) => {
                  e.currentTarget.src = 'https://img.icons8.com/ios-filled/50/ffffff/mosque.png';
                }}
              />
            </div>
            <div className="flex flex-col">
              <span className="text-xl font-bold leading-none">মসজিদ মহব্বত (Masjid Mohabbat)</span>
              {isOffline && (
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="flex items-center justify-between bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-3 py-2">
                <span className="text-[10px] font-bold text-[#64748b] uppercase">সব দেখুন</span>
                <Switch 
              ...
              filteredMosques.map((mosque) => (
                ...
                >
                  ...
                  <div className="flex justify-between text-[12px] text-[#64748b]">
                    <span>{mosque.distance || '0km'} দূরে</span>
                    <span>আসন্ন: {mosque.nextJamaat}</span>
                  </div>
                </div>
              ))
            ) : (
              <div className="text-center py-10 bg-[#f8fafc] border border-dashed border-[#e2e8f0] rounded-xl">
                <Search className="w-8 h-8 text-[#94a3b8] mx-auto mb-2 opacity-20" />
                <p className="text-[10px] text-[#64748b] font-bold">কোনো মসজিদ পাওয়া যায়নি</p>
                <Button 
                  variant="link" 
                  size="sm"
                  onClick={() => {
                  ...
                  className="text-[10px] text-[#065f46] font-bold h-auto p-0 mt-1"
                >
                  ফিল্টার রিসেট করুন
                </Button>
              </div>
      ...
      <div className="lg:hidden flex flex-col h-screen">
        ...
        <header className="bg-[#065f46] text-white py-2 px-4 md:px-6 flex items-center justify-between shadow-sm sticky top-0 z-50">
          <div className="flex items-center gap-2">
              ...
              <img 
                ...
                onError={(e) => {
                  ...
                  if (parent) {
                    ...
                    icon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 text-white"><path d="M2 20h20"/><path d="M7 20v-7a5 5 0 0 1 10 0v7"/><path d="M12 13V7"/><path d="M12 3v4"/><path d="M3 20v-3a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v3"/></svg>';
            ...
            <h1 className="font-bold text-base md:text-lg">মসজিদ মহব্বত <span className="hidden md:inline">(Masjid Mohabbat)</span></h1>
          </div>
        ...
        <main className="flex-1 overflow-hidden flex flex-col md:flex-row bg-background">
          {/* Mobile View: Tabs */}
          <div className="md:hidden flex-1 overflow-y-auto p-4 pb-24 bg-background">
            {isOffline && (
              ...
              <TabsContent value="mosques" className="space-y-6 outline-none mt-0">
                <div className="flex flex-col gap-5">
                  <div className="flex items-center justify-between px-1">
                    <div>
                      <h2 className="text-2xl font-black text-foreground">নিকটস্থ মসজিদ</h2>
                      <p className="text-xs text-muted-foreground">আপনার চারপাশের জামায়াতের সময়সূচী</p>
                    </div>
                  ...
                  {userRole === 'admin' && (
                    <Card className="bg-gradient-to-br from-[#065f46] to-[#047857] text-white border-none shadow-md rounded-2xl overflow-hidden">
                      <CardContent className="p-4 flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <Sparkles className="w-4 h-4 text-emerald-300" />
                            <h3 className="text-sm font-bold">মসজিদ ম্যানেজমেন্ট</h3>
                          </div>
                          <p className="text-[10px] text-emerald-100 opacity-90 leading-tight truncate">
                            নতুন মসজিদ তালিকাভুক্ত করুন বা আপডেট করুন।
                          </p>
                        </div>
                        <Button 
                          onClick={() => setActiveTab('dashboard')}
                          className="bg-white text-[#065f46] hover:bg-emerald-50 h-9 px-4 rounded-xl text-[10px] font-bold shrink-0 shadow-sm border-none"
                        >
                          <Plus className="w-3.5 h-3.5 mr-1" /> ম্যানেজ করুন
                        </Button>
                      </CardContent>
                    </Card>
                  )}
                  ...
                  {filteredMosques.length > 0 ? (
                    filteredMosques.map((mosque) => (
                      <Card 
                        ...
                        className={`bg-card border-border shadow-sm transition-all overflow-hidden rounded-2xl relative group active:scale-[0.98] ${selectedMosque?.id === mosque.id ? 'border-[#059669] ring-2 ring-[#059669]/10 shadow-md' : 'hover:border-[#cbd5e1]'} ${!mosque.active ? 'opacity-60 grayscale-[0.5]' : ''}`}
                      >
                        {!mosque.active && (
                          <Badge className="absolute top-3 right-3 bg-muted text-muted-foreground text-[9px] h-5 px-2 border-none font-bold">
                            নিষ্ক্রিয়
                          </Badge>
                        )}
                        <CardHeader className="pb-2 bg-gradient-to-r from-background to-card border-b border-border p-4">
                          <div className="flex justify-between items-start">
                            <div className="flex-1 min-w-0 pr-3">
                              <div className="flex items-center gap-2 mb-1">
                                <CardTitle className="text-base font-bold text-foreground leading-tight truncate">{mosque.name}</CardTitle>
                                {mosque.active && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0" />}
                              </div>
                                <CardDescription className="text-[11px] flex items-center gap-1.5 text-muted-foreground">
                    ...
                    <div className="text-center py-12 bg-white border border-dashed border-[#e2e8f0] rounded-2xl">
                      ...
                      <p className="text-sm text-[#64748b] font-medium">{radiusFilter === 'all' ? 'সব' : radiusFilter + ' কি.মি.'} এর মধ্যে কোনো মসজিদ পাওয়া যায়নি</p>
                      <Button 
                        variant="link" 
                        onClick={() => {
                          setRadiusFilter('all');
            ...
            <div className="w-80 border-r border-border bg-card flex flex-col">
              <div className="p-4 border-b border-border">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">নিকটস্থ মসজিদসমূহ</h3>
                </div>

                {userRole === 'admin' && (
                  <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 rounded-2xl shadow-sm">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-[11px] font-bold text-emerald-900 dark:text-emerald-300 leading-tight mb-1">প্রশাসনিক প্যানেল</p>
                        <p className="text-[10px] text-emerald-700 dark:text-emerald-400 leading-tight mb-3">নতুন মসজিদ যোগ করতে ড্যাশবোর্ড ব্যবহার করুন</p>
                        <Button 
                          size="sm"
                          onClick={() => setActiveTab('dashboard')}
                          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white h-7 text-[9px] rounded-lg font-bold shadow-sm"
                        >
                          <Plus className="w-3 h-3 mr-1" /> ড্যাশবোর্ড ওপেন করুন
                        </Button>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold text-muted-foreground uppercase">স্ট্যাটাস ও অবস্থান</Label>
                    <div className="space-y-2.5">
                       <select 
                        value={statusFilter}
                        onChange={(e) => setStatusFilter(e.target.value as any)}
                        className="w-full px-3 py-2 bg-[#f8fafc] border border-[#e2e8f0] rounded-xl text-[10px] font-bold text-[#065f46] appearance-none focus:ring-1 focus:ring-[#065f46] outline-none"
                      >
                        <option value="all">সব মসজিদ (সক্রিয় ও নিষ্ক্রিয়)</option>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="flex-[1.5] relative">
                        ...
                        <Input 
                          ...
                          className="w-full pl-8 pr-2 py-2 h-9 bg-background border border-border rounded-xl text-[10px] font-bold focus:ring-primary"
                        />
                      </div>
                    </div>
                  </div>
